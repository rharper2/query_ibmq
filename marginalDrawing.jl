...
"""
    translateLocation15(data,qubit)
...
"""
function translate_15Q_Location(data,i)
    a = Array{Float64}(undef,2,8)
    a[1,1:7]= data[1:7]
    a[2,1:8]=reverse(data[8:15])
    a[2,1]=-3
    #a[1,8]=-3
    return a
end
...
"""
function translate_15Q_toIndex(i)
   if i < 7
        return (i,1)
   end
   return (15-i,2)
...
"""
function drawIBM_15Q_Hinton(mutualPs,titleText;saveFile = "",titleFontSize=24)
    addScale = false
    fig = figure("Slightly larger",figsize=(18,6))
    scale = round(maximum(maximum.(mutualPs)/5),RoundUp,digits=3)
    fig[:suptitle](titleText*" Full box = $scale",fontsize=titleFontSize)
    ...
    for (ix,i) in enumerate(1:4)
      subplot(4,4,ix)
      ax=gca()
     hinton(translate_15Q_Location(mutualPs[i],i),scale,ax,highlightNegative=true,qubit=i-1)
    end
